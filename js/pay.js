var app = new Vue({
  ...
  methods: {
    ...
    var clientid=this.form.client_id;
      this.$http.get('/getinvoice/' + clientid)
            ...
            table.setAttribute('class','table table-striped');
             ...
             for(var row=0; row < data.length; row++){
                ...
                var span = document.createElement('span_'+invoiceId);
                 span.innerHTML = '<a class="btn btn-success" data-toggle="modal" id="button_'+invoiceId +'" href="#myModal10 " invoiceId="'+invoiceId +'" invoice_no="'+data[row].invoice_no +'"  amountleft="'+data[row].amountleft +'" grand_total="'+data[row].grand_total +'" onclick="makePayment(this)" text="Make Payment" >Make Payment</a';
    ...
    update: function() {
      ...
      this.isProcessing = true;
      var obj=this.form;
      this.$http.put('/payments/' + this.form.payment_id, this.form)
        .then(function(response) {
             $("#loader-wrapper").hide();
          window.location.href = '/payments';
